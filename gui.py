...
class App(ctk.CTk):
    ...
    # ---- 任务 2b: 账号密码登录 ----
    # 协议依据见 czn_lite.stove_password_field / signin_password 的注释。
    # 验证码无法自动解（click 点选 + rotate 旋转，都是 CV 任务），
    # 因此这里弹交互窗口由用户作答；协议细节全在 captcha.py。
    def _ui_sync(self, fn):
        """worker 线程里调用：把 fn 切到 UI 线程执行并等它的返回值。
    ...
    def _task_pwdlogin(self):
        ...
        if code2 == 49703:
            ...
            self.log_line("    ⇒ 说明该 token 未被接受：可能头名不对，或 token 已过期/被消费")
            self.log_line("      请把日志导出后反馈（见 docs/FINDINGS.md 的未定项）")
        self.set_status("状态: 登录失败")
